...
# def infix_to_postfix(expression): #1+2 -> 12+
    output = ""
    ...
    while i < len(expression):
        ...
        if char.isdigit():
            ...
            while i < len(expression) and expression[i].isdigit():
                ...
                i += 1
            output += number + " "
            i -= 1  #bc after while loop it overshot
        elif char == '(': #collecting shit afterwards
            operator_stack.push(char)
        elif char == ')':
            while not operator_stack.is_empty() and operator_stack.peek() != '(':
                output += operator_stack.pop() + " "
           
            if not operator_stack.is_empty() and operator_stack.peek() == '(': #remove '(' in the stack
                operator_stack.pop()
        elif char in PRECEDENCE:  # Check if character is an operator
            while (not operator_stack.is_empty() and
                   operator_stack.peek() != '(' and
                   has_higher_precedence(operator_stack.peek(), char)): #if op in stack has higher pre than curr then pop
                output += operator_stack.pop() + " "
...
def infix_to_postfix(expression):
    ...
    while(i < len(expression)):
        ...
        if(char.isdigit()):
            ...
            output += number + ' '
        if(char == ')'):
            while(opSt.top and opSt.peek()!='('):
                output += opSt.pop() + ' '
            opSt.pop() #removing '('
        elif(char == '('):
            opSt.push(char)
        elif(char in PRECEDENCE):
            while(opSt.top and has_higher_precedence(opSt.peek(), char)):
                output += opSt.pop() + ' '
            opSt.push(char)
        i+=1
    while(opSt.top):
