...
class HashTable:
    def __init__(self, num_data):
        self.load_factor = 3
        self.size = max(1, num_data // self.load_factor)  # Ensure at least one slot if 6 then 2 slots
    ...
    def hash_insert(self, key):
        ...
        if(self.hash_search(key)): #if key alr in slot
            return False
        
        slot = self._hash(key)
        newNode = Node(key)
        newNode.next = self.table[slot]
